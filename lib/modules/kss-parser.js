...
function jsonSections(sections, block) {
  return sections.map(function(section) {
    ...
    if (partial.markup() && partial.markup().toString().match(/^[^\n]+\.(html|hbs|pug)$/)) {
      ...
      // Override markup method
      var originalMarkup = section.markup;
    ...
    return {
      ...
      markup: section.markup ? (section.markup() === '' ? null : section.markup()) : null
    };
  });
}
...
function processBlock(block, options) {
  return Q.Promise(function(resolve, reject) {
    ...
    try {
      // Parse with KSS 3.x library (directly returns styleguide in KSS 3.x)
      var styleguide = kss.parse(block.sanitizedKss, options);
      var section = jsonSections(styleguide.sections(), block);
      ...
      if (section.length > 0) {
        ...
        var blockStyles = trimLinebreaks(block.code);
