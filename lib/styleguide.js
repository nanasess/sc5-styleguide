  ...
  return through(throughOpts, bufferFileContents, function(callback) {
      ...
      Q.all([parseKSSPromise, parseVariablesPromise]).spread(function(sections, variables) {
        ...
        stylesCompiled = Q.Promise(function(resolve) {
          ...
          // CSS変数ファイルを事前に結合するためのパス
          const combinedVariablesPath = distPath + '/css/_styleguide_combined_variables.css';
          // すべての変数ファイルを1つにまとめる
          try {
            let combinedContent = '';
            ...
            fs.writeFileSync(combinedVariablesPath, combinedContent);
          } catch (e) {
            console.error('Error combining variable files:', e);
          }
          ...
          try {
            ...
            variableFiles.forEach(file => {
              if (fs.existsSync(file)) {
                ...
                // SCSS変数構文に対応
                const matches = content.match(/\$([\w\-_]+):\s*([^;!]+)(?:!default)?;/g);
                if (matches) {
                  matches.forEach(match => {
                    const varMatch = match.match(/\$([\w\-_]+):\s*([^;!]+)(?:!default)?;/);
                    if (varMatch) {
...
// Helper function to remove KSS comment blocks
function stripKssComments(cssString) {
  // Regular expression to match KSS comment blocks
  // KSS comments have specific patterns like "Styleguide" references
  // Updated to handle multiline comments more reliably
  var kssCommentRegex = /\/\*[\s\S]*?Styleguide\s+[\d\.]+[\s\S]*?\*\//gm;
  
  // First remove KSS comment blocks
  var cleanedCss = cssString.replace(kssCommentRegex, '');
  
  // Also remove any standalone comment blocks that might contain markup or sg-angular-directive
  var markupCommentRegex = /\/\*[\s\S]*?(markup:|sg-angular-directive:)[\s\S]*?\*\//gm;
  cleanedCss = cleanedCss.replace(markupCommentRegex, '');
...
module.exports.applyStyles = function() {
  ...
  function bufferFileContents(file, enc, done) {
    ...
    // Get file content and strip KSS comments
    fileContent = file.contents.toString();
    var strippedContent = stripKssComments(fileContent);

    // Add styles to common stylesheet (without KSS comments)
    stylesBuffer += strippedContent;

    // Create stylesheet that contains pseudo styles
    pseudoStylesPromise = Q.Promise(function(resolve) {
      pseudoStylesBuffer += pseudoSelectors.stylesFromString(strippedContent, {
        source: file.path
      });
      resolve();
    });
    ...
    atRulesPromise = Q.Promise(function(resolve) {
      atRulesBuffer += atRules.stylesFromString(strippedContent, {
        source: file.path
      });
      resolve();
    });
